...
Will use HTML5 for this SASsession.""")
    ...
    def _breakprompt(self, eos):
        ...
        while True:
            ...
            if rc is not None:
                ...
                self._sb.SASpid = None
                outrc = str(rc)
                return dict(LOG=b'SAS process has terminated unexpectedly. Pid State= ' + outrc.encode(), LST=b'',
                            ABORT=True)
    ...
    def _break(self, inlst=''):
        ...
        while True:
            if len(lst) > 0:
                ...
                if found:
                    ll = self.submit(
                        "ods " + self.sascfg.output + " (id=saspy_internal) close;ods listing close;ods listing;libname work list;\n",
                        'text')
                    break
    ...
    def sasdata2dataframe(self, table: str, libref: str = '', dsopts: dict = None, rowsep: str = '\x01',
        ...
        code = ""
        code += "filename sock socket '" + host + ":" + str(port) + "' lrecl=" + str(
            self.sascfg.lrecl) + " recfm=v termstr=LF;\n"
