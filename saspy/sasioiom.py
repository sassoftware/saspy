...
Will use HTML5 for this SASsession.""")
        ...
        if os.name == 'nt':
            try:
                self.pid = subprocess.Popen(parms, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
        ...
        if os.name == 'nt':
            ...
            if rc[0] == 0:
                ...
                print("Java Error:\n" + error)
                print("SAS Connection failed. No connection established. Staus=" + str(
                    rc) + "  Double check your settings in sascfg_personal.py file.\n")
                print("Attempted to run program " + pgm + " with the following parameters:" + str(parms) + "\n")
                print(
                    "If no Java Error above, try running the following command (where saspy is running) manually to see if it's a problem starting Java:\n" + s + "\n")
                self.pid = None
        ...
        if not zero:
            if not self.sascfg.sspi:
                while len(pw) == 0:
                    pw = self.sascfg._prompt("Please enter the password for IOM user " + self.sascfg.omruser + ": ",
                                             pw=True)
                    if pw is None:
                        if os.name == 'nt':
                            self.pid.kill()
        ...
        ll = self.submit(
            "options svgtitle='svgtitle'; options validvarname=any pagesize=max nosyntaxcheck; ods graphics on;",
            "text")
    ...
    def submit(self, code: str, results: str = "html", prompt: dict = {}) -> [dict, str]:
        '''
...
>>>>>>>
        ...
        while not done:
            try:
                ...
                self.pid = None
                log = logf.partition(logcodeo)[
                          0] + b'\nConnection Reset: SAS process has terminated unexpectedly. Pid State= ' + str(
                    rc).encode()
                return dict(LOG=log.decode(errors='replace'), LST='')
    ...
    def write_csv(self, file: str, table: str, libref: str = "", nosub: bool = False, dsopts: dict = {},
                  opts: dict = {}) -> 'The LOG showing the results of the step':
        """
...
>>>>>>>
    ...
    def sasdata2dataframe(self, table: str, libref: str = '', dsopts: dict = {}, rowsep: str = '\x01',
        ...
        code = "proc sql; create view sasdata2dataframe as select * from " + tabname + self._sb._dsopts(
            dsopts) + ";quit;\n"
...
>>>>>>>
        ...
        code = "data _null_; set " + tabname + self._sb._dsopts(
                ...
                if len(data) > 0:
                    ...
                    if len(r) > trows:
                        ...
                        for i in range(nvars):
                            if vartype[i] == 'N':
                                if varcat[
                                    i] not in self._sb.sas_date_fmts + self._sb.sas_time_fmts + self._sb.sas_datetime_fmts:
      ...
      dsopts = dsopts if dsopts is not None else {}
        ...
        code = "data sasdata2dataframe / view=sasdata2dataframe; set " + tabname + self._sb._dsopts(
            dsopts) + ";\nformat "
        ...
        if self.sascfg.iomhost.lower() in ('', 'localhost', '127.0.0.1'):
            local = True
            outname = "_tomodsx"
            code = "filename _tomodsx '" + tmpcsv + "' lrecl=" + str(
                self.sascfg.lrecl) + " recfm=v  encoding='utf-8';\n"
        else:
            local = False
