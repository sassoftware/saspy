...
class SASsession():
    ...
    **Common functions that can be used in the Notebook sessions supported by the `Display` Config setting**
    ...
    def sd2pl(self, table: str, libref: str = '', dsopts: dict = None,
              method: str = 'MEMORY', **kwargs) -> 'polars.DataFrame':
        """
    ...
    def polars2sasdata(self, df: 'polars.DataFrame', table: str = '_df', libref: str = '',
        ...
        # Convert LazyFrame to DataFrame for now, until streaming upload is implemented
        if hasattr(df, 'collect'):
           streaming = kwargs.get('streaming', False)
           df = df.collect(streaming=streaming)

        # We'll leverage the existing streaming architecture by converting to a format 
        # the access method can handle, or implementing it in the access method.
        # For now, let's convert to pandas if possible or implement in IO.
        
        # If the access method has polars2sasdata, use it.
        if hasattr(self._io, 'polars2sasdata'):
           res = self._io.polars2sasdata(df, table, libref, **kwargs)
        else:
           # Fallback: convert to pandas and use existing logic
