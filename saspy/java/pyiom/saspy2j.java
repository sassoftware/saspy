    ...
    public static void main(String[] args)  throws InterruptedException, IOException, ConnectionFactoryException
    {
        ...
        for(int x=0; x < nargs; x++)
        {
            if (args[x].equalsIgnoreCase("-host") )
                ...
                if (args[x].equalsIgnoreCase("-stdinport") )
                    inport  = Integer.parseInt(args[x+1]);
                else
                    if (args[x].equalsIgnoreCase("-stdoutport") )
                        outport = Integer.parseInt(args[x+1]);
                    else
                        if (args[x].equalsIgnoreCase("-stderrport") )
                            errport = Integer.parseInt(args[x+1]);
                        else
                            if (args[x].equalsIgnoreCase("-iomhost") )
                                iomhost = args[x+1];
                            else
                                if (args[x].equalsIgnoreCase("-iomport") )
        ...
        //System.out.print("Socket in  connected? "+sin.isConnected()+'\n');
        //System.out.print("Socket out connected? "+sout.isConnected()+'\n');
        //System.out.print("Socket err connected? "+serr.isConnected()+'\n');
        ...
        try
        {
            ...
            fileref = filesvc.AssignFileref("_tomods1", "", filesvc.FullName("tomods1", physicalName.value[0]), "", retname);
        ...
        while(true)
        {
            try
            {
                ...
                while (true)
                {
                    if ((idx = pgm.indexOf("tom says EOL=")) >= 0 &&
                               pgm.length() > idx+13+32)
                    {
