...
of the sections below. This section is to document how the data type conversion is done. It's pretty
straight forward, but not specifically doc'ed in an easy to find place; so here it is.

SAS has only 2 data types; fixed width character and floating point double. So, any data type in a Pandas
dataframe will be converted to one of those two when loading it into a SAS Data Set. When loading a dataframe
from SAS, character columns will be appropriately transcoded and loaded as strings (Object in Pandas).
Doubles will be loaded as float.

Date, time, timestamp (datetime) are the other types that are converted, though they aren't actual
datatypes in SAS, and Pandas only has datetime64, so moving between those is handled, and there's more
details about how to control this is the sections below.

Also, regardless of the type, SAS missing values will be NaN or NaT in the dataframe, and NaN/NaT will
become missing values in SAS.
...
.. code-block:: text
    ...
    double with the following type of SAS format
...
.. code-block:: text
    ...
    numeric         double
    datetime64      double with datetime format (or date/time if you override)
    str(*)          character  (any other type in Pandas)
...
As of Version 3.7.0, the MEMORY (default) version of sasdata2dataframe now outperforms all of the various methods.
...
a separate configuration definition (in your sascfg_personal.py) for each one. A good naming
