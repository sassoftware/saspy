...
So, in this case, the 'undo' won't happen so saspy won't have it's LOG back. In this case, you could code it in your program
before each 'abort cancel;' that could execute. 
...
Now, odds are that if you are submitting code like this, you didn't type it into saspy. You probably are reading in some existing
...
or if there has already been 'proc printo;run;' previously submitted.

.. code-block:: ipython3

    >>> fd = open('MyBattchSreipt.sas'); code = fd.read(); fd.close()
    >>> sas.submitLOG(code)
    >>> sas.submitLOG("proc printto;run;")
