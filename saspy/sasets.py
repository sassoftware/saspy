...
class SAS_stat:
    ...
    def _makeProccallMacro(self,objtype,objname,kwargs):
        ...
        crosscorr     =kwargs.get('crosscorr', '')
        decomp     =kwargs.get('decomp', '')
        id     =kwargs.get('id','')
        season     =kwargs.get('season', '')
        trend     =kwargs.get('trend', '')
        var     =kwargs.get('var', '')
        crossvar     =kwargs.get('crossvar', '')
        ...
        if len(by):
            logger.debug("by statement,length: " + by + "," + str(len(by)))
            code += "by %s;" % (by)
        if len(corr):
            logger.debug("corr statement,length: " + corr + "," + str(len(corr)))
            code += "corr %s;" % (corr)
        if len(crosscorr):
            logger.debug("crosscorr statement,length: " + crosscorr + "," + str(len(crosscorr)))
            code += "crosscorr %s;" % (crosscorr)
        
        if len(id):
            logger.debug("id statement,length: " + var + "," + str(len(id)))
            code += "id %s;" % (id)
        if len(var):
            logger.debug("var statement,length: " + var + "," + str(len(var)))
            code += "var %s;" % (var)            
